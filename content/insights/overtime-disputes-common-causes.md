---
title: "The five causes of every overtime dispute"
date: "2026-09-04"
category: "Attendance"
...
metaDescription: "Nearly every overtime argument traces to one of five causes — rounding, shift-end confusion, weekly-off rates, missed punch-outs."
readTime: 5
faqs:
  ...
  - q: "How does rounding affect overtime pay?"
    a: "Rounding rules — for example, rounding every punch to the nearest 15 minutes — save a few minutes here and there that add up over a month. Workers notice the pattern even when management doesn't. The fix is to round consistently in both directions, or not round at all and let the system calculate to the minute."
  - q: "What happens if an employee forgets to punch out?"
    a: "Without a record, someone has to guess the end time — usually the shift's scheduled end, which may be wrong in either direction. A missed punch-out should be flagged as an exception immediately, not discovered at month-end, so it can be corrected from a real signal like a gate camera or a supervisor's confirmation while everyone still remembers the shift."
  - q: "Should unapproved overtime be paid?"
    a: "That's a policy decision, not a system one — but the policy has to be written down and applied the same way every time. The dispute isn't usually about whether unapproved hours get paid; it's about workers finding out the rule mid-argument instead of before they stayed late."
  - q: "Why do weekly-off and holiday overtime rates cause disputes?"
    a: "Because the multiplier is easy to get wrong by hand and hard to verify without the raw data. A worker who worked a weekly-off shift wants to see the day marked correctly and the rate applied automatically, not recalculated manually by whoever is doing payroll that week."
---
