...
export async function POST(request: NextRequest) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (id) {
          ...
          // The register is a mirror of a delivered lead, not a fallback for
          // one. It stays on the success path so the shared sheet is a
          // complete record rather than a list of the failures only.
  ...
  // Two nets, not one. The durable intake above takes the lead when it is
  // configured; this is the other one, and it needs only a Sheets URL and a
  // secret rather than a database. Whichever is set catches the enquiry.
  //
  // The ERP is down, not the enquiry. If the register takes the row and both
  // recipients are alerted, the lead is captured and someone will call — so
  // the customer is told it arrived, and the ERP failure is carried in the
