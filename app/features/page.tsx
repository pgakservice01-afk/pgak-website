...
/** Derived, so adding a feature cannot leave a stale number on the page. */
const FEATURE_COUNT = EXPLORER_FEATURES.length;

export const metadata: Metadata = pageMeta({ title:`${FEATURE_COUNT} AI CCTV Features for Indian Businesses | PGAK`, description:`Explore ${FEATURE_COUNT} AI camera capabilities: smart search, intrusion alerts, ANPR, PPE, occupancy and more. See Indian use cases and camera compatibility requirements.`, path:'/features' });
export default function FeaturesPage() {
  return <><JsonLd nodes={[webPageSchema({path:'/features',name:`${FEATURE_COUNT} AI CCTV capabilities for Indian businesses`,description:'Explore AI video analytics capabilities, Indian use cases and hardware requirements.'}),breadcrumbSchema([{name:'Home',path:'/'},{name:'Features',path:'/features'}])]} /><Nav /><main id="main-content" className={styles.page}>
    <section className={styles.hero}>
      <div className={styles.heroCopy}><p className={styles.breadcrumb}><Link href="/">Home</Link><span>/</span>Features</p><p className={styles.kicker}>A MORE INTELLIGENT VIEW</p><h1>AI CCTV features.<br />Made clear.</h1><p className={styles.lead}>Find the moment. Understand the activity.<br />Help your team act sooner.</p><p className={styles.description}>Explore {FEATURE_COUNT} AI CCTV camera features for Indian businesses. See how video analytics can help with intrusion alerts, number plate recognition, people counting and faster footage searches.</p><div className={styles.actions}><a href="#explore-features" className="btn btn-primary">Explore all {FEATURE_COUNT} features ↓</a><Link href="/free-audit" className={styles.textLink}>Check my cameras ↗</Link></div><p className={styles.note}>The right capabilities depend on your cameras, hardware and software. We confirm compatibility before recommending a setup.</p></div>
