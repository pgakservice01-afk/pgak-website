...
'weapon-detection': {
keyword:'AI weapon detection camera',
intro:'A weapon visible at an entrance is a situation where minutes matter, and where reviewing footage afterwards is no help at all. Weapon detection analytics watch for the visual signature of a weapon held in view and raise an alert so a person can look and decide. The judgement stays human; the software only shortens the time before someone is looking.',
advice:'Treat this as an alerting aid attached to a response plan, not a safeguard on its own. Agree who receives the alert, what they check and what they do next, before it is switched on. Test it in your own lighting and camera angles: a tool held at waist height in a crowded frame is a much harder case than a clear demonstration, and you should see both before you buy.',
question:'Can it detect a concealed weapon?',answer:'No. Video analytics can only work with what the camera can see, so something inside a bag or under clothing is not detectable. This is not a screening or scanning system and should never be described to staff as one.'},
'abandoned-object': {
keyword:'Abandoned object detection CCTV',
intro:'Two opposite problems share one technique: something left behind that should not be there, and something removed that should be. Both are detected by learning what a monitored area normally looks like and flagging a change that persists. A bag in a lobby corner and a missing trolley from a marked bay are the same kind of event to the software.',
advice:'Pick areas where "normal" is genuinely stable. A corner that is empty all day works; a corridor where stock moves constantly will alert all day until the thresholds are loosened so far that they catch nothing. Set the dwell time deliberately, and expect a tuning period on your own site before the alerts are worth acting on.',
question:'Will it alert every time someone puts a box down?',answer:'It will until it is tuned. The threshold is how long an object must stay before it counts, and that number has to be set per area against your own activity. Expect to adjust it during the first fortnight rather than accepting a default.'},
'two-way-audio': {
keyword:'Two way audio CCTV camera',
intro:'Watching someone where they should not be, with no way to say anything, is a frustrating kind of security. Two-way audio puts a speaker and microphone at the camera position so whoever is watching can speak — to direct a delivery driver at a closed gate, or to make it plain to someone on the boundary that they have been seen.',
advice:'Decide what the speaker is for before choosing one: a short spoken instruction needs far less than intelligible speech across a yard, and volume and placement follow from that. Agree what staff may say, so a deterrent does not become a confrontation. Keep recording audio a separate decision from live speaking, because it carries obligations that video does not.',
question:'Can I add this to my existing cameras?',answer:'Sometimes. Some cameras carry audio input and output already; others need a separate network speaker at that position, plus power and network to it. Which applies is confirmed for each position during the assessment rather than assumed.'},
'onvif-integration': {
keyword:'ONVIF compatible VMS',
intro:'Most established sites are not one brand. A few cameras were added in 2018, more after an incident, and the recorder came from whoever was cheapest that year. Open interfaces such as ONVIF exist so those devices can be reached by software that did not ship with them, which is what makes a mixed estate workable instead of a reason to start again.',
advice:'Ask for specifics rather than the word "ONVIF" on its own. Which profile, on which device, for which function — a camera that streams video over a standard interface may still not pass its analytics events, its PTZ control or its audio the same way. Get the supported profiles and API functions named per device in the proposal, and test the two or three oldest cameras first, because those are where the surprises are.',
question:'Does ONVIF support mean everything will work?',answer:'No. ONVIF stream support and full analytics-event interoperability are different things, and a device can have one without the other. Confirm the exact profiles and functions for your models rather than relying on a compatibility badge.'},
};
